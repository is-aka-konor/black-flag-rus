import { convertDistance, formatDistance } from "../utils/_module.mjs";

/**
 * Extension of the token ruler to display movement distances.
 */
export default class BlackFlagTokenRuler extends foundry.canvas.placeables.tokens.TokenRuler {
	/** @inheritDoc */
	...
	_getWaypointLabelContext(waypoint, state) {
		...
		if (
			...
			next &&
			waypoint.actionConfig.visualize &&
			next.actionConfig.visualize &&
			waypoint.action === next.action &&
			(waypoint.unreachable || !waypoint.next.unreachable)
		)
			return;
		...
		// Segment Distance
		context.distance = { total: formatDistance(waypoint.measurement.distance.toNearest(0.01), canvas.grid.units) };
		...
		context.cost = {
			total: Number.isFinite(cost) ? formatDistance(cost.toNearest(0.01), canvas.grid.units) : "∞",
			units: ""
		};
		...
		context.elevation = {
			total: formatDistance(elevation, canvas.grid.units),
	 ...
	 * @param {TokenRulerWaypoint} waypoint - The waypoint to style.
	...
	#getSpeedBasedStyle(waypoint, style) {
		...
		if (!movement?.types || !this.token.actor?.system.isCreature) return style;
		...
		const { normal, double, triple } = CONFIG.BlackFlag.tokenRulerColors;
		const increment = (waypoint.measurement.cost - 0.1) / currActionSpeed;
